...
ERR XMLNode::AddAttribute(std::shared_ptr<XMLAttribute>&& attribute)
{
	auto result = std::find_if(attributes.begin(), attributes.end(), [&](std::shared_ptr<XMLAttribute> const& attr) {return strcmp(attribute->GetName().c_str(), attr->GetName().c_str()) == 0; });
...
ERR XMLNode::AddAttribute(const char* attrName, const char* attrValue)
{
	...
	auto result = std::find_if(attributes.begin(), attributes.end(), [&](std::shared_ptr<XMLAttribute> const& attr) {return strcmp(attr->GetName().c_str(), attrName) == 0; });
...
{
	auto attribute = std::find_if(attributes.begin(), attributes.end(), [&](std::shared_ptr<XMLAttribute> const&  attr) {return strcmp(attrName.c_str(), attr->GetName().c_str()) == 0; });
...
{
	return std::find_if(attributes.begin(), attributes.end(), [&](std::shared_ptr<XMLAttribute> const& attr) {return strcmp(attr->GetName().c_str(), attrName.c_str()) == 0; }) != attributes.end();
}
